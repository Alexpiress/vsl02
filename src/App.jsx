...
// ── API ───────────────────────────────────────────────────
async function callAPI(system, user, max_tokens = 1000) {
  ...
  // Se o backend conseguiu parsear, retorna o objeto
  if (data.parsed) return { ok: true, data: data.parsed, raw: data.text };
  // Senão retorna o texto puro (para copys)
  return { ok: true, data: null, raw: data.text };
}
...
const CSS = `
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .drop-zone{border:2px dashed #2A2A40;border-radius:12px;padding:24px;text-align:center;cursor:pointer;transition:border-color 0.2s,background 0.2s;}
  .drop-zone:hover,.drop-zone.over{border-color:#7C5CFC;background:#7C5CFC10;}
  .img-thumb{width:100%;max-height:200px;object-fit:cover;border-radius:10px;border:1px solid #2A2A40;display:block;}
`;
      ...
      <div className={`drop-zone${over?" over":""}`}
        ...
        {value?.url ? (
          <div>
            ...
            <div style={{fontSize:11,color:T.muted,fontFamily:"sans-serif",marginTop:8}}>Clique para trocar</div>
          </div>
        ) : (
          <div>
            <div style={{fontSize:28,marginBottom:8}}>📸</div>
            <div style={{fontSize:13,color:T.textSub,fontFamily:"sans-serif"}}>Arraste o printscreen ou clique para selecionar</div>
            <div style={{fontSize:11,color:T.muted,fontFamily:"sans-serif",marginTop:6}}>A IA lê a imagem para identificar Enneagrama e Big Idea</div>
          </div>
        )}
      </div>
    ...
    <div style={{...mk.ic(e.cor),flex:1,minWidth:220}}>
      ...
      {bigIdeia && (
        <div style={{background:e.cor+"18",borderRadius:8,padding:10,marginBottom:8}}>
          <div style={{fontSize:9,fontWeight:700,color:e.cor,letterSpacing:"0.07em",fontFamily:"sans-serif",marginBottom:4}}>💡 BIG IDEA</div>
          <div style={{fontSize:12,color:T.text,fontFamily:"sans-serif",lineHeight:1.5}}>{bigIdeia}</div>
        </div>
      )}
      {evidencias && (
        <div style={{fontSize:11,color:T.muted,fontFamily:"sans-serif",fontStyle:"italic",borderTop:`1px solid ${T.border}`,paddingTop:8}}>{evidencias}</div>
      )}
    </div>
  );
}
  ...
  async function analisar() {
    ...
    try {
      ...
      try {
        ...
        siteTexto = d.contents?.replace(/<[^>]+>/g," ").replace(/\s+/g," ").slice(0,2000) || "";
      } catch(_) {}

      let printBase64=null, printMediaType="image/jpeg";
      if (prodPrint?.file) {
        setLoadMsg("Processando printscreen...");
        printBase64 = await fileToBase64(prodPrint.file);
        printMediaType = prodPrint.file.type || "image/jpeg";
      }

      const siteCtx = [
        siteTexto ? `TEXTO DO SITE:\n${siteTexto}` : "",
        prodExtra ? `EXTRAS:\n${prodExtra}` : "",
      ].filter(Boolean).join("\n\n");

      setLoadMsg("Analisando VSL e produto...");

      // Prompt com JSON simples sem caracteres especiais nos valores
      const system = `Você é especialista em copywriting e Enneagrama. Responda SOMENTE com JSON. Sem texto antes ou depois. Sem markdown. Sem aspas internas nos valores — use apenas texto simples sem pontuação complexa.`;

      const userContent = printBase64 ? [
        { type:"image", source:{ type:"base64", media_type:printMediaType, data:printBase64 } },
        { type:"text", text:`VSL:\n${transcricao.slice(0,3000)}\n\nPRODUTO: ${prodNome}\nURL: ${prodURL}\n${siteCtx}\n\nResponda APENAS com este JSON (sem aspas dentro dos valores de string):\n{"tipoVSL":3,"evidVSL":"evidencia aqui","bigIdeiaVSL":"big idea aqui","tipoProduto":2,"evidProduto":"evidencia aqui","bigIdeiaProduto":"big idea aqui","alinhados":false,"gap":"descricao gap","oportunidade":"descricao oportunidade","caminhoA":"como aplicar caminho A","caminhoB":"como aplicar caminho B","caminhoC":"tipo ponte e justificativa","cena":"cena identificada","acao":"acao e queda","espelho":"espelho coletivo","solucao":"solucao","promessa":"promessa dupla","objecao":"objecao","urgencia":"urgencia","mecanismo":"mecanismo","personagem":"perfil narrador"}` }
      ] : `VSL:\n${transcricao.slice(0,3000)}\n\nPRODUTO: ${prodNome}\nURL: ${prodURL}\n${siteCtx}\n\nResponda APENAS com este JSON (sem aspas dentro dos valores de string):\n{"tipoVSL":3,"evidVSL":"evidencia aqui","bigIdeiaVSL":"big idea aqui","tipoProduto":2,"evidProduto":"evidencia aqui","bigIdeiaProduto":"big idea aqui","alinhados":false,"gap":"descricao gap","oportunidade":"descricao oportunidade","caminhoA":"como aplicar caminho A","caminhoB":"como aplicar caminho B","caminhoC":"tipo ponte e justificativa","cena":"cena identificada","acao":"acao e queda","espelho":"espelho coletivo","solucao":"solucao","promessa":"promessa dupla","objecao":"objecao","urgencia":"urgencia","mecanismo":"mecanismo","personagem":"perfil narrador"}`;

      const result = await callAPI(system, userContent, 1000);

      if (!result.data) {
        // Tenta parsear manualmente como último recurso
        throw new Error("A IA não retornou JSON válido. Tente novamente.");
      }

      const d = result.data;
      // Normaliza estrutura flat para o formato esperado
      const dados = {
        tipoVSL: Number(d.tipoVSL) || 3,
        evidVSL: d.evidVSL || "",
        bigIdeiaVSL: d.bigIdeiaVSL || "",
        tipoProduto: Number(d.tipoProduto) || 3,
        evidProduto: d.evidProduto || "",
        bigIdeiaProduto: d.bigIdeiaProduto || "",
        alinhados: d.alinhados === true || d.alinhados === "true",
        gap: d.gap || "",
        oportunidade: d.oportunidade || "",
        caminhos: { A: d.caminhoA||"", B: d.caminhoB||"", C: d.caminhoC||"" },
        caesp: {
          cena: d.cena||"", acao: d.acao||"", espelho: d.espelho||"",
          solucao: d.solucao||"", promessa: d.promessa||"",
          objecao: d.objecao||"", urgencia: d.urgencia||"",
          mecanismo: d.mecanismo||"", personagem: d.personagem||"",
        }
      };
  ...
  async function gerarCopy(novaVersao=false) {
    ...
    try {
      ...
      const caminhoCtx = !analise.alinhados && caminho
        ? `Caminho ${caminho}: ${analise.caminhos?.[caminho]||""}`
        : "Alinhados — use a sinergia.";
...
Português brasileiro. Tom par-a-par. Confissão pessoal. Narrativa real.
Mini VSL e Social: indicações de cena entre [colchetes].
VSL Completa: blocos nomeados.
ESCREVA APENAS A COPY.`;

      const result = await callAPI(system, user, 1000);
      const texto = result.raw || "";

      if (novaVersao) {
        setVersoes(v=>[...v,{texto,tipo:tipoFinal,fmt:fmt?.label}]);
      } else {
        setCopyGerada(texto);
        setVersoes([]);
        setFase(4);
    ...
    <div style={{minHeight:"100vh",background:T.bg,color:T.text,fontFamily:"Georgia,serif"}}>
      ...
      <div style={{background:`linear-gradient(135deg,${T.surface},#0D0D1A)`,borderBottom:`1px solid ${T.border}`,padding:"20px 28px",display:"flex",alignItems:"center",gap:14}}>
        ...
        <div>
          ...
          <div style={{fontSize:11,color:T.muted,fontFamily:"sans-serif",marginTop:2}}>Engenharia reversa de copy · C.A.E.S.P.+ · Enneagrama · Big Idea</div>
        </div>
          ...
          <div style={mk.card()}>
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:18}}>
              ...
              <div>
                ...
                <div style={{fontSize:11,color:T.muted,fontFamily:"sans-serif",marginTop:2}}>Transcrição ou link do YouTube</div>
              </div>
            </div>
            <div style={{display:"flex",gap:10,marginBottom:14}}>
              {["texto","link"].map(m=>(
                <button key={m} disabled={fase>1} onClick={()=>setModoVSL(m)} style={mk.chip(modoVSL===m,T.accent)}>
                  {m==="texto"?"📝 Colar transcrição":"▶ Link YouTube"}
                </button>
              ))}
            </div>
            {modoVSL==="texto"?(
              <><label style={mk.lbl}>Transcrição</label>
              <textarea value={vslTexto} onChange={e=>setVslTexto(e.target.value)}
                placeholder="Cole o texto completo da VSL..." style={mk.ta(8)} disabled={fase>1}/></>
            ):(
              <><label style={mk.lbl}>Link YouTube</label>
          ...
          <div style={mk.card()}>
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:18}}>
              ...
              <div>
                ...
                <div style={{fontSize:11,color:T.muted,fontFamily:"sans-serif",marginTop:2}}>Nome, URL, print e informações extras</div>
              </div>
            </div>
            ...
            {fase===1 ? (
              ...
              <div style={{marginBottom:14}}>
                <label style={mk.lbl}>Print do site</label>
                <img src={prodPrint.url} alt="site" className="img-thumb" style={{maxHeight:120}}/>
              </div>
            ) : null}
            ...
            <textarea value={prodExtra} onChange={e=>setProdExtra(e.target.value)}
              placeholder="Cole textos, diferenciais, posicionamento do produto..." style={mk.ta(4)} disabled={fase>1}/>
          </div>

          {fase===1&&(
            <div style={{textAlign:"center"}}>
              {erro&&<div style={mk.err}>{erro}</div>}
              <button onClick={analisar} style={mk.btn(`linear-gradient(135deg,${T.accent},${T.accentB})`,"#fff",{padding:"14px 52px",fontSize:14})}>
            ...
            <div style={mk.card()}>
              <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
                ...
                <div>
                  ...
                  <div style={{fontSize:11,color:T.muted,fontFamily:"sans-serif",marginTop:2}}>Big Idea · Enneagrama · Mapa · C.A.E.S.P.+</div>
                </div>
              </div>
              ...
              {analise.alinhados ? (
                <div style={mk.ic(T.green)}>
                  <span style={mk.tag(T.green)}>✓ ALINHADOS</span>
                  <p style={{fontSize:13,color:T.text,fontFamily:"sans-serif",marginTop:8,marginBottom:0}}>{analise.oportunidade}</p>
                </div>
              ) : (
                <>
                  <div style={mk.ic(T.red)}>
                    <span style={mk.tag(T.red)}>⚡ GAP DETECTADO</span>
                    <p style={{fontSize:13,color:T.text,fontFamily:"sans-serif",marginTop:8,marginBottom:0}}>{analise.gap}</p>
                  </div>
                  <div style={mk.ic(T.gold)}>
                  ...
                  <div style={{marginTop:14}}>
                    ...
                    {CAMINHOS.map(c=>(
                      <div key={c.id} onClick={()=>setCaminho(caminho===c.id?null:c.id)}
                        style={{...mk.ic(caminho===c.id?T.accent:T.border),cursor:"pointer",border:`1px solid ${caminho===c.id?T.accent:T.border}`,transition:"all 0.15s"}}>
                        ...
                        {analise.caminhos?.[c.id]&&<p style={{fontSize:12,color:T.textSub,fontFamily:"sans-serif",margin:0}}>{analise.caminhos[c.id]}</p>}
                      </div>
                    ))}
                  </div>
                </>
                ...
                <>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                    {[["C — Cena",caesp.cena],["A — Ação",caesp.acao],["E — Espelho",caesp.espelho],["S — Solução",caesp.solucao],["P — Promessa",caesp.promessa],["O — Objeção",caesp.objecao],["U — Urgência",caesp.urgencia],["Mecanismo",caesp.mecanismo]].map(([k,v])=>(
                      <div key={k} style={{background:T.surface,borderRadius:8,padding:10,border:`1px solid ${T.border}`}}>
                        <div style={{fontSize:9,fontWeight:700,color:T.accent,fontFamily:"sans-serif",letterSpacing:"0.06em",marginBottom:4}}>{k}</div>
                        <div style={{fontSize:11,color:T.textSub,fontFamily:"sans-serif",lineHeight:1.5}}>{v||"—"}</div>
                      </div>
                    ))}
                  </div>
                </>
            ...
            <div style={{...mk.card(),border:`1px solid ${T.accent}`}}>
              <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:18}}>
                ...
                <div>
                  ...
                  <div style={{fontSize:11,color:T.muted,fontFamily:"sans-serif",marginTop:2}}>Formato · Enneagrama de destino</div>
                </div>
              </div>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:7,marginBottom:20}}>
                {Object.entries(ENNEAGRAM).map(([num,e])=>(
                  <button key={num} onClick={()=>setEnneaDest(enneaDest===+num?null:+num)} style={mk.chip(enneaDest===+num,e.cor)} title={e.driver}>
              ...
              {erro&&<div style={mk.err}>{erro}</div>}
              <button onClick={()=>gerarCopy(false)} style={mk.btn(`linear-gradient(135deg,${T.accent},${T.accentB})`,"#fff",{padding:"13px 40px",fontSize:14})}>
                ✦ Gerar Copy
              </button>
            </div>
          </div>
          ...
          <div className="fu" ref={resultRef}>
            <div style={mk.card()}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:12}}>
                  ...
                  <div>
                    ...
                    <div style={{fontSize:11,color:T.muted,fontFamily:"sans-serif",marginTop:2}}>
                      {FORMATOS.find(f=>f.id===formato)?.label} · Tipo {enneaDest||analise?.tipoVSL} — {ENNEAGRAM[enneaDest||analise?.tipoVSL]?.nome}
                    </div>
                  </div>
                </div>
                <button onClick={()=>copiar(copyGerada)} style={mk.btn(copiado?T.green:T.surface,copiado?"#fff":T.textSub,{border:`1px solid ${copiado?T.green:T.border}`,padding:"8px 18px",fontSize:12})}>
              ...
              <div style={{display:"flex",gap:10,marginTop:14,flexWrap:"wrap"}}>
                <button onClick={()=>gerarCopy(true)} style={mk.btn(`linear-gradient(135deg,${T.accent},${T.accentB})`,"#fff",{padding:"10px 22px",fontSize:13})}>
                  ↻ Nova versão · mesmo Enneagrama
                </button>
                <button onClick={()=>{setCopyGerada("");setVersoes([]);}} style={mk.btn("transparent",T.textSub,{border:`1px solid ${T.border}`,padding:"10px 20px",fontSize:13})}>
                  ← Mudar configurações
                </button>
              </div>
            </div>
                ...
                {versoes.map((v,i)=>(
                  <div key={i} style={{...mk.card(),marginBottom:12}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                      ...
                      <button onClick={()=>copiar(v.texto)} style={mk.btn(T.surface,T.textSub,{border:`1px solid ${T.border}`,padding:"6px 14px",fontSize:11})}>📋 Copiar</button>
                    </div>
                    <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:10,padding:16}} className="copybox">{v.texto}</div>
                  </div>
                ))}
              </div>
            )}
